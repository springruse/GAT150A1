...
void Ally::Update(float deltaTime)
{
	...
	Enemy* enemy = owner->m_scene->GetActorByName<Enemy>("enemy");
	...
	if (enemy) {
		...
		if (enemy) {
			...
			direction = enemy->owner->m_transform.position - owner->m_transform.position;
...
void Ally::Read(const claw::json::value_t& value)
{
	Object::Read(value);
